...
def calcular_metricas_sinal(filtered_signal, fps):
    """Calcula métricas temporais e espectrais para avaliar o sinal.

    As métricas incluem desvio-padrão, amplitude, energia, pico espectral,
    razão sinal-ruído, razão pico/ruído e concentração espectral na faixa cardíaca.
    """
    ...
    fft_magnitude = (2.0 / len(signal)) * np.abs(np.fft.rfft(signal))
